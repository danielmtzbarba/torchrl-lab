...
from utils.racing import eval_model, get_norm_stats, make_dqn_model, make_env
...
@hydra.main(
    config_path="configs/", config_name="config_racing.yaml", version_base="1.1"
)
def main(cfg: "DictConfig"):  # noqa: F821
    ...
    # Make the components
    model, model_explore, greedy_module = make_dqn_model(cfg)

    #    obs_norm_std = get_norm_stats()

    # Create the collector
    collector = SyncDataCollector(
        create_env_fn=make_env(env_name=cfg.env.env_name),
    ...
    if cfg.logger.backend:
        exp_name = generate_exp_name("DQN", f"{cfg.env.env_name}")
        logger = get_logger(
            cfg.logger.backend,
            logger_name="dqn",
    ...
    test_env = make_env(
        env_name=cfg.env.env_name, device="cpu", from_pixels=cfg.logger.video
    )
    if cfg.logger.video:
